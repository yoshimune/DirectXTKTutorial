...
void Game::Render()
{
	...
	m_d3dContext->OMSetBlendState(m_status->Opaque(), nullptr, 0xFFFFFFFF);
	m_d3dContext->OMSetDepthStencilState(m_status->DepthNone(), 0);

	//m_d3dContext->RSSetState(m_status->CullNone());
...
void Game::CreateDevice()
{
	...
	m_status = std::make_unique<CommonStates>(m_d3dDevice.Get());
	...
	CD3D11_RASTERIZER_DESC rastDesc(D3D11_FILL_SOLID, D3D11_CULL_NONE, FALSE, D3D11_DEFAULT_DEPTH_BIAS,
		D3D11_DEFAULT_DEPTH_BIAS_CLAMP, D3D11_DEFAULT_SLOPE_SCALED_DEPTH_BIAS, TRUE, FALSE, TRUE, FALSE);
